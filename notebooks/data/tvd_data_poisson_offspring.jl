...
function run_simulation(
    ...
    for K ∈ K_vals
        ...
        cbp_transition_probs = TVDAlgorithm.transition_probabilities(
            TVDAlgorithm.CBPKPoissonOffspring(K)
        )
        psdbp_transition_probs = TVDAlgorithm.transition_probabilities(
            TVDAlgorithm.PSDBPMatchingKPoissonOffspring(K, mean_only)
        )
...
run_simulation(
    rng, K_vals, path_length, num_trials, "K", false, "out/data/tvd_poisson_offspring_z0_is_K.json"
)

run_simulation(
    rng, K_vals, path_length, num_trials, 1, false, "out/data/tvd_poisson_offspring_z0_is_1.json"
)

run_simulation(
    rng, K_vals, path_length, num_trials, 1, true, "out/data/tvd_poisson_offspring_mean_only_z0_is_1.json"
)
